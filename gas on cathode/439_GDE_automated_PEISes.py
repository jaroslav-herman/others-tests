...
import pandas as pd
    ...
    # %%
files = we.load_files(r'\\ELECTROLYZER\PEM-WE_measurements\2026\439_III_III_IrOxonPTL_150nm_Pt_500ug_Pressures_N115_etchedcathode_BDC929\PEISes',['_PEIS','sccm_automated'],omit_string = ['_0_sccm','1.5_sccm'],natural_sort=True)
print(files)
...
for file,color,label in zip(files,colors,flows):
    ...
    averaged = data.groupby('cycle number')[['<Ewe>/V', '<I>/mA']].mean().reset_index()
    plt.plot(averaged['<Ewe>/V'], averaged['<I>/mA'], '-', c=color, label=label)
plt.legend()
plt.show()
# %%
...
for cycle in cycles:
    ...
    params = [0,0, 0.01,1e-8,  0.3, 0.1,  0.9, 0.3, 0.2,  0.9]
    bounds = (  [0,   1e-10, 0,   1e-5, 0.5, 0,   0.1, 0.5],
                [0.05,1e-5,  2,   1,    1,   2,   10,   1])
    ...
    for file,color,flow in zip(files,colors,flows):
        ...
        f,Z,E,I = weis.freq_and_Z(group,cycle,[0,10000])
    #    print(E,I)
        init = params[2:]

        cir = "R0-L0-p(R1,CPE1)-p(R2,CPE2)"

        params,errors = weis.fit_spectrum(f, Z, cir="R0-L0-p(R1,CPE1)-p(R2,CPE2)", init=init, bounds=bounds, E=np.round(E,3), I=I, tau_sort=False)
        # print(params)
...
for (flow,group),c in zip(params_flow,colors):
    plt.plot(group['I'],group['R2'],c=c,label = flow)
plt.legend()
plt.xscale('log')
plt.yscale('log')
plt.ylim(1e-2,5)
