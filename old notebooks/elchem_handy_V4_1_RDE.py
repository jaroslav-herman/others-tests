...
def dayproc(filelist):
    ...
    for filef in filelist:
        # Extract the first number as the day
        day_match = re.search(r'\d+', filef)
        # Extract the last two letters before .mpt
        procedure_match = re.search(r'(\w\w)\.mpt$', filef)
        
        if day_match and procedure_match:
            day = int(day_match.group())
            procedure = procedure_match.group(1)
            
            if day not in day_procedures:
                day_procedures[day] = []
            if procedure not in day_procedures[day]:
                day_procedures[day].append(procedure)
    
    shortlist = []
    for day in sorted(day_procedures.keys()):
    ...
    for n in qfilelist:
        if n.endswith('.mpt'):
            ...
            with open(str(n), 'r', encoding="cp855") as f:
                                first_line = f.readline()f.readline()
...
def select_MEA(data,filelist,framenum=[-6,-5],\
           frame = 'all'):      # Select only certain scans or frames, returns data and filelist array
               
    tf=np.zeros(len(data))
    if frame == 'all':
        fnum = []
        for i,filef in enumerate(filelist):
            fnum.append(str(filef[framenum[0]:framenum[1]]))
            frame = set(fnum)

    for i,filef in enumerate(filelist):
        fnum = str(filef[framenum[0]:framenum[1]])
        if (fnum in frame):
            tf[i]=1
            tf[i+1]=1
                #print(j)
...
#     plt.close("all")
    ...
    for i,filef in enumerate(filelist):
        ...
        dnum =int(numbers[0])
        snum=numbers[-1]
        if (dnum in day) and ((filef[typen[0]:typen[1]])==procedure) and (int(numbers[1]) in proc_nr) and (int(snum) in seq):
            tf[i]=1
                #print(j)
...
def selectProc(data, filelist, day='all', procedure='SV', loop=[1, 2, 3], proc_nr=[1], seq='all', 
               typen=[-6, -4], EIS_E='all', fR=[3E4, 1]):
    firstSV = 81
    lastSV = 220
    llengthSV = 361
    llengthCA = 1200
    ...
    # Extract days if 'all' is specified
    if day == 'all':
        day = {int(re.findall(r'\d+', filef)[0]) for filef in filelist if re.findall(r'\d+', filef)}

    # Extract sequences if 'all' is specified
    if seq == 'all':
        seq = {int(re.findall(r'\d+', filef)[-1]) for filef in filelist if re.findall(r'\d+', filef)}

    # Process each file and mark those that match the criteria
    for i, filef in enumerate(filelist):
        numbers = re.findall(r'\d+', filef)
        if len(numbers) < 2:  # Ensure there are enough numerical parts
            print(f"Skipping file with unexpected format: {filef}")
            continue
        dnum = int(numbers[0])
        snum = int(numbers[-1])
        proc_name = filef[typen[0]:typen[1]]
        
        if (dnum in day) and (proc_name == procedure) and (int(numbers[1]) in proc_nr) and (snum in seq):
            tf[i] = True
...
def plotData(datasets, labels, colors, areas, procedure, plot_params, folder, gradient='Y', merge=False):
    """
    Plots the processed data based on the specified procedure for multiple datasets.
    

    Args:
        datasets (list of list of np.ndarray): List of processed data lists from selectProc function.
        labels (list of str or list of list of str): List of labels for each dataset, either a single string or a list of strings.
        colors (list of str): List of colors for each dataset.
        areas (list of float): List of surface areas for normalization for each dataset.
        procedure (str): The procedure type (SV, CA, IS, MB).
        plot_params (dict): Dictionary containing plot parameters (font sizes, axis ranges, tick settings, min length).
        folder (str): Folder path to save the exported plot.
        gradient (str): 'Y' to apply gradient colors within a dataset, 'N' for solid colors (default: 'Y').
    """
    def generate_gradient_color(base_color, num_curves, factor=0.7):
        """
        Generate a gradient of colors based on the base color.

        Args:
            base_color (str): The base color to generate the gradient from.
            num_curves (int): The number of gradient colors to generate.
            factor (float): The range limiting factor to avoid extreme colors (default: 0.7).

        Returns:
            list of str: A list of gradient colors.
        """
    ...
    fig, ax = plt.subplots()
    added_labels = set()  # Track added labels to avoid duplicates
   ...
   counter_timer=0
    ...
    for dataset_idx, (dataset, label, base_color, S) in enumerate(zip(datasets, labels, colors, areas)):
        ...
        # Create a gradient of colors if gradient is 'Y'
        if gradient == 'Y':
        ...
        for curve_idx, (data, color) in enumerate(zip(dataset, gradient_colors)):
            ...
            # Determine the label for each curve
            ...
            if procedure == 'SV':
                ax.plot(data[front:back, 8] / (S * 1000), data[front:back, 7], label=plot_label, color=color, linewidth=plot_params.get('linewidth', 1))
            elif procedure == 'CA':
                ax.plot(data[front:back, 7] / 3600, data[front:back, 10] / (S * 1000), label=plot_label, color=color, linewidth=plot_params.get('linewidth', 1))
            elif procedure == 'IS':
                ax.set_aspect('equal', 'box')
                ax.plot(data[front:back, 1] * S, data[front:back, 2] * S, label=plot_label, color=color, linewidth=plot_params.get('linewidth', 1))
            elif procedure == 'MB':
                ax.plot(data[front:back, 8] / 3600+counter_time, data[front:back, 11] / (S * 1000), label=plot_label, color=color, linewidth=plot_params.get('linewidth', 1))
    ...
    for dataset, user_params in zip(data_processed, user_params_list):
        ...
        for curve, R in zip(dataset, R_ohm):
        	...
        	# Extend R_ohm list if fewer values are provided than curves
                ...
                corrected_dataset.append(corrected_curve)
            elif base_params['procedure'] == 'PEIS':
...
def EIS_fit(data,filelist,loop,circ='R_0-p(R_1,CPE_1)',initial=[0.01220557, 0.06365453, 0.31149243, 0.62819865], bound=([0.01,0,0,0.6],[np.inf,np.inf,np.inf,1])):
    ...
    for i,filef in enumerate(filelist):
        if len(data[i])==0:
            continue
        if len(data[i])>=3*llengthIS*0.9:
                dataL=np.array_split(data[i], 3)
                loops.append(3)
        elif (len(data[i])<3*llengthIS*0.9) and (len(data[i])>=1*llengthIS*1.1):
                dataL=np.array_split(data[i], 2)
                loops.append(2)
        else:
            dataL=[data[i]]
            loops.append(1)
        for j in range(0,len(dataL)):
            circuit=CustomCircuit(initial_guess=initial, circuit=circ, name=filef)
            circuit.fit(dataL[j][:,0], np.vectorize(complex)(dataL[j][:,1],-1*dataL[j][:,2]), bounds=bound)
            fitted.append([circuit.predict(dataL[j][:,0]).real,-circuit.predict(dataL[j][:,0]).imag])
            parameters.append(vars(circuit)['parameters_'])
            param_names.append(circuit.get_param_names())
            circuits.append(circuit)
    return fitted,parameters,param_names, loops