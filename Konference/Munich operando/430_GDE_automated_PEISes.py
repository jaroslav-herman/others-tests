...
# %%
files = we.load_files(
    r"C:\Users\Herman\OneDrive - Univerzita Karlova\katodovy oblouk\Data z PEMWE\430\Different H2 flows II",
    ["_PEIS", "sccm_automated"],
    extension=".mpr",
    omit_string=[
        "_301_sccm",
    ],
...
for file, color, label in zip(files, colors, flows):
    ...
    averaged = (
        data.groupby("cycle number")[["<Ewe>/V", "<Ece>/V", "<I>/mA"]]
        .mean()
        .reset_index()
    )
    # print(averaged)
    plt.plot(
        averaged["<Ewe>/V"] - averaged["<Ece>/V"],
        averaged["<I>/mA"],
        "-",
        c=color,
        label=label,
...
for cycle in cycles:
    ...
    bounds = ([0, 1e-10, 0, 1e-5, 0.7, 0, 1e-5, 0.7], [0.05, 1e-5, 2, 10, 1, 2, 10, 1])
    ...
    for file, color, flow in zip(files, colors, flows):
        ...
        params, errors = weis.fit_spectrum(
            f,
            Z,
            cir="R0-L0-p(R1,CPE1)-p(R2,CPE2)",
            init=init,
...
params_all["C1"] = weis.capacitance(
    params_all["R1"], params_all["Q1"], params_all["a1"]
)
params_all["C2"] = weis.capacitance(
    params_all["R2"], params_all["Q2"], params_all["a2"]
)
...
# %%
plt.plot(
    [int(flow) for flow, q in intersections], [q for flow, q in intersections], "-o"
)
plt.grid()
