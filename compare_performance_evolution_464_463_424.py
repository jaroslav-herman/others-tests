"""Compare performance evolution of samples 464, 463, and 424 in 2026/AEM."""
...
def find_sample_folders() -> dict[str, Path]:
    """Find one AEM measurement folder for each requested sample."""
    found: dict[str, Path] = {}
...
def current_evolution(folder: Path) -> dict[float, list[float]]:
    """Return current nearest each selected voltage, in natural file order."""
    files = we.load_files(
        str(folder),
        contains_string="SV",
        extension=".mpr",
...
def main() -> None:
    ...
    for target_voltage in CELL_VOLTAGES:
        ...
        fig.suptitle(f"Performance evolution at {target_voltage:g} V")
        fig.tight_layout()

        output = OUTPUT_DIR / (
            f"performance_evolution_464_463_424_{target_voltage:g}V.png"
        )
